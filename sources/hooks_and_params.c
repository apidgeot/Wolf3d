...
int		collision_up(t_wolf *data)
{
	if (collision_chkr(data->ppos.px + data->ppos.pdx, data->ppos.py,
	data) == 0)
	{
		if (collision_chkr(data->ppos.px, data->ppos.py + data->ppos.pdy,
		data) == 0)
			return (0);
		else
		{
	...
	else if (collision_chkr(data->ppos.px, data->ppos.py + data->ppos.pdy,
	data) == 0)
	{
	...
	if (collision_chkr(data->ppos.px + data->ppos.pdx, data->ppos.py +
	data->ppos.pdy, data) == 0)
		return (0);
	return (1);
}

int		collision_down(t_wolf *data)
{
	if (collision_chkr(data->ppos.px - data->ppos.pdx, data->ppos.py,
	data) == 0)
	{
...
int		collision(int key, t_wolf *data)
{
	if (key == ARROW_UP)
	{
		if (collision_up(data) == 0)
			return (0);
	}
	if (key == ARROW_DOWN)
	{
		if (collision_down(data) == 0)
			return (0);
	}
	return (1);
}
...
int		do_key(int key, void *param)
{
	...
	data = (t_wolf *)param;
	key2(key, data);
	if ((collision(key, data) == 1 || data->ppos.invis_val == 1))
		key1(key, data);
	if (key == L_SHIFT)
		data->ppos.invis_val = 1;
	if (key == M_KEY)
